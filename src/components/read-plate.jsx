...
const LicensePlateReader = ({ onPlateConfirm, onReset }) => {
    ...
    const { getVehicleFromEntity, vehicleByEntity } =
    ...
    useEffect(() => {
        ...
        console.log(
            "Component mounted, video element:",
            videoElement ? "disponibile" : "non disponibile"
        );
    }, []);
    ...
    const startCamera = async () => {
        ...
        setTimeout(async () => {
            try {
                ...
                console.log(
                    "Accesso alla fotocamera ottenuto, stream:",
                    stream
                );
                if (videoRef.current) {
    ...
    const capturePhoto = async () => {
        ...
        if (!videoRef.current) {
            ...
            setError(
                "Impossibile acquisire l'immagine: riferimento al video non disponibile"
            );
            return;
        }
        if (!canvasRef.current) {
            ...
            setError(
                "Impossibile acquisire l'immagine: riferimento al canvas non disponibile"
            );
            return;
        }
        try {
            ...
            console.log(
                `Dimensioni video originali: ${videoWidth}x${videoHeight}`
            );
            if (videoWidth === 0 || videoHeight === 0) {
                console.error("Dimensioni video non valide");
                setError(
                    "Impossibile acquisire l'immagine: flusso video non valido"
                );
                return;
            }
            ...
            if (!targetRect) {
                ...
                setError(
                    "Impossibile acquisire l'immagine: riferimento al rettangolo non disponibile"
                );
                return;
            }
            ...
            const sourceHeight = Math.min(
                videoHeight,
                rectBounds.height * scaleY
            );
            console.log(
                `Area di ritaglio nel video: x=${sourceX}, y=${sourceY}, w=${sourceWidth}, h=${sourceHeight}`
            );
    ...
    const analyzeImageData = async (imageData) => {
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(
                    `Errore dal server: ${response.status} ${errorText}`
                );
            }
    ...
    const confirmManualPlate = () => {
        ...
        let formattedPlate = manualPlate
            .toUpperCase()
            .replace(/[^A-Z0-9]/g, "");
        if (formattedPlate.length >= 3) {
            formattedPlate =
                formattedPlate.slice(0, 2) + " " + formattedPlate.slice(2);
        }
