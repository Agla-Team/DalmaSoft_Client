...
export default function LeftPanel({ onTelaioChange, refreshTrigger }) {
  ...
  const handleConfirmInventory = () => {
    ...
    if (response) {
      setAlertMessage(response.message || "Auto confermata con successo!");
      setAlertOpen(true);
      resetForm();
    }
  }
      ...
      <div className="mb-6">
        <Card className="w-full border border-slate-700 rounded-t">
          ...
          <CardContent className="pt-2">
            ...
            <Button onClick={handleConfirmInventory} className="w-full bg-amber-500 text-gray-900 hover:bg-amber-600 pt-2">
              Conferma Auto
            </Button>
          </CardContent>
        </Card>
